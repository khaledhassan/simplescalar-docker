/* Subroutines for insn-output.c for SimpleScalar
   Contributed by A. Lichnewsky, lich@inria.inria.fr.
   Changes by     Michael Meissner, meissner@osf.org.
   Copyright (C) 1989, 1990, 1991 Free Software Foundation, Inc.
   Much hacking by Todd M. Austin
...
rtx ss_load_reg, ss_load_reg2, ss_load_reg3, ss_load_reg4;
...
enum processor_type ss_cpu;

/* which instruction set architecture to use.  */
int ss_isa;

/* Strings to hold which cpu and instruction set architecture to use.  */
char *ss_cpu_string;		/* for -mcpu=<xxx> */
char *ss_isa_string;		/* for -ss{1,2,3} */

/* Generating calls to position independent functions?  */
enum ss_abicalls_type ss_abicalls;
...
/* Array to RTX class classification.  At present, we care about
   ...
   peephole code.  */
char ss_rtx_classify[NUM_RTX_CODE];

/* Array giving truth value on whether or not a given hard register
   can support a given mode.  */
char ss_hard_regno_mode_ok[(int)MAX_MACHINE_MODE][FIRST_PSEUDO_REGISTER];

/* Current frame information calculated by compute_frame_size.  */
struct ss_frame_info current_frame_info;

/* Zero structure to initialize current_frame_info.  */
struct ss_frame_info zero_frame_info;

/* Temporary filename used to buffer .text until end of program
   for -mgpopt.  */
static char *temp_filename;

/* List of all SS punctuation characters used by print_operand.  */
char ss_print_operand_punct[256];

/* Map GCC register number to debugger register number.  */
int ss_dbx_regno[FIRST_PSEUDO_REGISTER];
...
/* Hardware names for the registers.  If -mrnames is used, this
   will be overwritten with ss_sw_reg_names.  */
...
enum reg_class ss_regno_to_class[] =
{
...
enum reg_class ss_char_to_class[256] =
{
     ...
     enum machine_mode mode;
{
  ...
  if (TARGET_SS_AS)		/* gas doesn't like li.d/li.s yet */
    {
     ...
     enum machine_mode mode;
{
  ...
  if (GET_MODE_SIZE (GET_MODE (op)) > 8)
    return FALSE;
  ...
  switch (GET_CODE (addr))
	...
	return TRUE;
      ...
      else if (TARGET_REG_PLUS_REG
	       && GET_CODE(plus1) == REG
	       && GET_CODE(plus0) == REG)
	return TRUE;

      else if (USE_INC_DEC
	       && (GET_CODE(plus0) == PRE_DEC
		   || GET_CODE(plus0) == POST_DEC
		   || GET_CODE(plus0) == PRE_INC
		   || GET_CODE(plus0) == POST_INC)
	       && ((GET_CODE(plus1) == CONST_INT
		    && SMALL_INT (plus1))
		   || (TARGET_REG_PLUS_REG
		       && GET_CODE(plus1) == REG)))
	return TRUE;
	       ...
	       && (GET_CODE(plus1) == PRE_DEC
		   || GET_CODE(plus1) == POST_DEC
		   || GET_CODE(plus1) == PRE_INC
		   || GET_CODE(plus1) == POST_INC)
	       && ((GET_CODE(plus0) == CONST_INT
		    && SMALL_INT (plus0))
		   || (TARGET_REG_PLUS_REG
		       && GET_CODE(plus0) == REG)))
	...
	return FALSE;
	 ...
	 interfere if the SS linker rewrites the store instruction
    ...
    case CONST:
	 ...
	 Also, save some time if not optimizing.  */
      if (ss_section_threshold == 0 || !optimize || !TARGET_GP_OPT)
	return FALSE;
...
int
real_simple_memory_operand (op, mode)
     rtx op;
     enum machine_mode mode;
{
  rtx addr, plus0, plus1;
  ...
  switch (GET_CODE (addr))
	...
	return FALSE;
      ...
      else if ((GET_CODE(plus0) == PRE_DEC
		|| GET_CODE(plus0) == POST_DEC
		|| GET_CODE(plus0) == PRE_INC
		|| GET_CODE(plus0) == POST_INC)
	       && ((GET_CODE(plus1) == CONST_INT
	...
	return FALSE;
      ...
      else if ((GET_CODE(plus1) == PRE_DEC
		|| GET_CODE(plus1) == POST_DEC
		|| GET_CODE(plus1) == PRE_INC
		|| GET_CODE(plus1) == POST_INC)
	       && ((GET_CODE(plus0) == CONST_INT
...
/* Return an operand string if the given instruction's delay slot or
   ...
   on its own.  For the SS assembler, all we do is update the filled
...
char *
ss_fill_delay_slot (ret, type, operands, cur_insn)
     ...
     rtx cur_insn;		/* current insn */
{
  ...
  ss_load_reg  = set_reg;
  ss_load_reg2 = (mode == DImode || mode == DFmode)
			? gen_rtx (REG, SImode, REGNO (set_reg) + 1)
			: (rtx)0;

  if (type == DELAY_HILO)
    {
      ss_load_reg3 = gen_rtx (REG, SImode, MD_REG_FIRST);
      ss_load_reg4 = gen_rtx (REG, SImode, MD_REG_FIRST+1);
    }
  else
    {
      ss_load_reg3 = 0;
...
char *
ss_move_1word (operands, insn, unsignedp)
     rtx operands[];
     rtx insn;
     int unsignedp;
{
      ...
      else if (code1 == SYMBOL_REF || code1 == CONST)
	{
	  if (HALF_PIC_P () && CONSTANT_P (op1) && HALF_PIC_ADDRESS_P (op1))
	      ...
	      if (GET_CODE (op1) == SYMBOL_REF)
		{
		  ...
		  if (TARGET_STATS)
		    ss_count_memory_refs (operands[2], 1);
  ...
  if (delay != DELAY_NONE)
    return ss_fill_delay_slot (ret, delay, operands, insn);
     ...
     rtx insn;
{
  ...
  if (code0 == REG)
      ...
      if (code1 == REG)
	{
	  ...
	  /* FIXME: could these cause a cross-endian problem?? */
      ...
      else if (code1 == CONST_DOUBLE)
	{
	  if (CONST_DOUBLE_HIGH (op1) != 0 || CONST_DOUBLE_LOW (op1) != 0)
		...
		{
		  /* FIXME: could this cause a cross-endian problem?? */
     ...
     rtx addr;
{
  switch (GET_CODE (addr))
	...
	switch (GET_CODE (plus1))
	  ...
	  case HIGH:
	  case LO_SUM:
	    return ss_address_cost (plus1) + 1;
	  }
      }
    }
     ...
     enum block_move_type move_type;
{
  ...
  if (num_regs > 2 && (bytes > 2*align || move_type != BLOCK_MOVE_NORMAL))
	...
	{
	  if (TARGET_STATS)
	    ss_count_memory_refs (operands[1], 1);
	...
	{
	  if (TARGET_STATS)
	    ss_count_memory_refs (operands[0], 1);
  ...
  while (bytes > 0)
      ...
      if (TARGET_STATS && move_type != BLOCK_MOVE_LAST)
	{
	  ...
	  if (CONSTANT_P (src_reg))
	    ss_count_memory_refs (src_reg, 1);

	  if (CONSTANT_P (dest_reg))
	    ss_count_memory_refs (dest_reg, 1);
	}
     ...
     rtx libname;		/* SYMBOL_REF of library name or 0 */
{
  ...
  *cum = zero_cum;
  ...
  /* Determine if this function has variable arguments.  This is
     ...
     are no variable arguments.  The standard SS calling sequence
     ...
     int named;			/* != 0 for normal args, == 0 for ... args */
{
  ...
  if (cum->arg_words >= MAX_ARGS_IN_REGISTERS)
	...
	fprintf (stderr, "%s%s\n", reg_names[regbase + cum->arg_words + bias],
	 ...
	 the same way that the SS compiler does (namely by passing
	 ...
	 See the CUMULATIVE_ARGS definition in ss.h.  */
     ...
     FILE *out;
{
  ...
  int j;
  char **p;
  int mask = target_flags /* TARGET_DEFAULT */;
  ...
  fprintf (out, "\n\n # Cc1 arguments (-G value = %d, Cpu = %s, ISA = %d):",
	   ss_section_threshold, ss_cpu_string, ss_isa);
...
override_options ()
{
  ...
  /* pre- and post- inc/dec options */
  if (ss_inc_dec_opt_string) {
    ss_inc_dec_opt = atoi(ss_inc_dec_opt_string);
    fprintf(stderr,
	    "INC_DEC_OPTS: pre-inc:%c post-inc:%c pre-dec:%c post-dec:%c\n",
	    USE_PRE_INCREMENT ? 't' : 'f',
	    USE_POST_INCREMENT ? 't' : 'f',
	    USE_PRE_DECREMENT ? 't' : 'f',
	    USE_POST_DECREMENT ? 't' : 'f');
  }

  /* parse max-align flag, if we got it */
  if (ss_max_align_string) {
    ss_max_alignment = atoi(ss_max_align_string)*8;
    if (ss_max_alignment < 4*8)
      fatal("max-align value of `%d' is too small (must be >= 4)",
	    ss_max_alignment/8);
    if ((ss_max_alignment & (ss_max_alignment-1)) != 0)
      fatal("max-align value of `%d' is not a power of 2", ss_max_alignment/8);
    fprintf(stderr, "ss_max_alignment = %d bytes\n", ss_max_alignment/8);
  }

  if (ss_stack_align_string) {
    ss_stack_alignment = atoi(ss_stack_align_string)*8;
    if (ss_stack_alignment < 4*8)
      fatal("max-stack-align value of `%d' is too small (must be >= 4)",
	    ss_stack_alignment/8);
    if ((ss_stack_alignment & (ss_stack_alignment-1)) != 0)
      fatal("max-stack-align value of `%d' is not a power of 2",
	    ss_stack_alignment/8);
    fprintf(stderr, "ss_stack_alignment = %d bytes\n", ss_stack_alignment/8);
  }

  ss_section_threshold =
    (g_switch_set) ? g_switch_value : SS_DEFAULT_GVALUE;

  /* Identify the processor type */
  if (ss_cpu_string == (char *)0
      || !strcmp (ss_cpu_string, "default")
      || !strcmp (ss_cpu_string, "DEFAULT"))
    {
      ss_cpu_string = "default";
      ss_cpu = PROCESSOR_DEFAULT;
    }

  else
    {
      char *p = ss_cpu_string;
	...
	case '4':
	  if (!strcmp (p, "4000") || !strcmp (p, "4k") || !strcmp (p, "4K"))
	    ss_cpu = PROCESSOR_R4000;
	  break;

	case '6':
	  if (!strcmp (p, "6000") || !strcmp (p, "6k") || !strcmp (p, "6K"))
	    ss_cpu = PROCESSOR_R6000;
	  break;
	}

      if (ss_cpu == PROCESSOR_DEFAULT)
	{
	  error ("bad value (%s) for -mcpu= switch", ss_cpu_string);
	  ss_cpu_string = "default";
	}
    }

  /* Now get the architectural level.  */
  if (ss_isa_string == (char *)0)
    ss_isa = 1;

  else if (isdigit (*ss_isa_string))
    ss_isa = atoi (ss_isa_string);

  else
    {
      error ("bad value (%s) for -ss switch", ss_isa_string);
      ss_isa = 1;
    }

  if (ss_isa < 0 || ss_isa > 3)
    error ("-ss%d not supported", ss_isa);

  else if (ss_isa > 1
	   && (ss_cpu == PROCESSOR_DEFAULT || ss_cpu == PROCESSOR_R3000))
    error ("-mcpu=%s does not support -ss%d", ss_cpu_string, ss_isa);

  else if (ss_cpu == PROCESSOR_R6000 && ss_isa > 2)
    error ("-mcpu=%s does not support -ss%d", ss_cpu_string, ss_isa);
  ...
  if (TARGET_ABICALLS)
    ss_abicalls = SS_ABICALLS_YES;
  else
    ss_abicalls = SS_ABICALLS_NO;

  /* -mrnames says to use the SS software convention for register
     names instead of the hardware names (ie, a0 instead of $4).
     We do this by switching the names in ss_reg_names, which the
  ...
  if (TARGET_NAME_REGS)
      ...
      if (TARGET_GAS)
	{
	  ...
	  warning ("Gas does not support the SS software register name convention.");
	}
      bcopy ((char *) ss_sw_reg_names, (char *) ss_reg_names, sizeof (ss_reg_names));
    }
  ...
  ss_rtx_classify[(int)PLUS]  = CLASS_ADD_OP;
  ss_rtx_classify[(int)MINUS] = CLASS_ADD_OP;
  ss_rtx_classify[(int)DIV]   = CLASS_DIVMOD_OP;
  ss_rtx_classify[(int)MOD]   = CLASS_DIVMOD_OP;
  ss_rtx_classify[(int)UDIV]  = CLASS_DIVMOD_OP | CLASS_UNSIGNED_OP;
  ss_rtx_classify[(int)UMOD]  = CLASS_DIVMOD_OP | CLASS_UNSIGNED_OP;
  ss_rtx_classify[(int)EQ]    = CLASS_CMP_OP | CLASS_EQUALITY_OP | CLASS_FCMP_OP;
  ss_rtx_classify[(int)NE]    = CLASS_CMP_OP | CLASS_EQUALITY_OP | CLASS_FCMP_OP;
  ss_rtx_classify[(int)GT]    = CLASS_CMP_OP | CLASS_FCMP_OP;
  ss_rtx_classify[(int)GE]    = CLASS_CMP_OP | CLASS_FCMP_OP;
  ss_rtx_classify[(int)LT]    = CLASS_CMP_OP | CLASS_FCMP_OP;
  ss_rtx_classify[(int)LE]    = CLASS_CMP_OP | CLASS_FCMP_OP;
  ss_rtx_classify[(int)GTU]   = CLASS_CMP_OP | CLASS_UNSIGNED_OP;
  ss_rtx_classify[(int)GEU]   = CLASS_CMP_OP | CLASS_UNSIGNED_OP;
  ss_rtx_classify[(int)LTU]   = CLASS_CMP_OP | CLASS_UNSIGNED_OP;
  ss_rtx_classify[(int)LEU]   = CLASS_CMP_OP | CLASS_UNSIGNED_OP;

  ss_print_operand_punct['?'] = TRUE;
  ss_print_operand_punct['#'] = TRUE;
  ss_print_operand_punct['&'] = TRUE;
  ss_print_operand_punct['!'] = TRUE;
  ss_print_operand_punct['*'] = TRUE;
  ...
  ss_char_to_class['d'] = GR_REGS;
  ss_char_to_class['f'] = ((TARGET_HARD_FLOAT) ? FP_REGS : NO_REGS);
  ss_char_to_class['h'] = HI_REG;
  ss_char_to_class['l'] = LO_REG;
  ss_char_to_class['x'] = MD_REGS;
  ss_char_to_class['y'] = GR_REGS;
  ss_char_to_class['z'] = ST_REGS;
      ...
      for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
	{
	  ...
	  if (mode == CC_FPmode || mode == CC_REV_FPmode)
	    ...
	    temp = ((TARGET_FLOAT64 || ((regno & 1) == 0))
			...
			|| (0 /* TARGET_DEBUG_H_MODE */ && class == MODE_INT)));
	  ...
	  ss_hard_regno_mode_ok[(int)mode][regno] = temp;
	}
    }
}
...

/*
 * The SS debug format wants all automatic variables and arguments
 * to be in terms of the virtual frame pointer (stack pointer before
 * any adjustment in the function), while the SS 3.0 linker wants
...
int
ss_debugger_offset (addr, offset)
     rtx addr;
     int offset;
{
  ...
  /* sdbout_parms does not want this to crash for unrecognized cases.  */
    ...
    abort_with_insn (addr, "ss_debugger_offset called with non stack/frame/arg pointer.");
#endif
     ...
     rtx addr;
{
  ...
  if (!addr)
	...
	if (!USE_PRE_INCREMENT)
	  abort_with_insn (addr, "PRINT_OPERAND_ADDRESS, PRE_INC unimpl.");
	istr = "0(%s)^+";
	goto inc_dec_targ;
      case POST_INC:
	if (!USE_POST_INCREMENT)
	  abort_with_insn (addr, "PRINT_OPERAND_ADDRESS, POST_INC unimpl.");
	istr = "0(%s)+";
	goto inc_dec_targ;
      case PRE_DEC:
	if (!USE_PRE_DECREMENT)
	  abort_with_insn (addr, "PRINT_OPERAND_ADDRESS, PRE_DEC unimpl.");
	istr = "0(%s)^-";
	goto inc_dec_targ;
      case POST_DEC:
	if (!USE_POST_DECREMENT)
	  abort_with_insn (addr, "PRINT_OPERAND_ADDRESS, POST_DEC unimpl.");
	istr = "0(%s)-";
	goto inc_dec_targ;

      inc_dec_targ:
	if (!USE_INC_DEC)
	  abort_with_insn (addr, "INC_DEC requested but not enabled");
	if (GET_CODE (XEXP(addr, 0)) != REG)
	  abort_with_insn (addr, "INC_DEC target not a register.");
	if (REGNO (XEXP(addr, 0)) == ARG_POINTER_REGNUM)
	  abort_with_insn (addr, "Arg pointer not eliminated.");
	fprintf(file, istr, reg_names [REGNO (XEXP(addr, 0))]);
	break;

      case PLUS:
	{
	  register rtx inc    = (rtx)0;
	  register rtx reg    = (rtx)0;
	  register rtx offset = (rtx)0;
	  register rtx arg0   = XEXP (addr, 0);
	  register rtx arg1   = XEXP (addr, 1);

	  if (GET_CODE (arg0) == PRE_INC ||
	      GET_CODE (arg0) == POST_INC ||
	      GET_CODE (arg0) == PRE_DEC ||
	      GET_CODE (arg0) == POST_DEC)
	    {
	      inc = arg0;
	      reg = XEXP(arg0, 0);
	      offset = arg1;
	  ...
	  else if (GET_CODE (arg1) == PRE_INC ||
              	   GET_CODE (arg1) == POST_INC ||
              	   GET_CODE (arg1) == PRE_DEC ||
              	   GET_CODE (arg1) == POST_DEC)
	    {
	      inc = arg1;
	      reg = XEXP(arg1, 0);
	      offset = arg0;
		...
		abort_with_insn (addr, "Arg pointer not eliminated.");
	      ...
	      if (TARGET_REG_PLUS_REG && GET_CODE (offset) == REG) {
		if (GET_CODE(inc) == PRE_INC) {
		  if (!USE_PRE_INCREMENT)
	            abort_with_insn (addr,
				     "PRINT_OPERAND_ADDRESS, PRE_INC unimpl.");
		  fprintf (file, "(%s+%s)^+",
			   reg_names [REGNO(reg)], reg_names [REGNO(offset)]);
		}
		else if (GET_CODE(inc) == POST_INC) {
		  if (!USE_POST_INCREMENT)
		    abort_with_insn (addr,
				     "PRINT_OPERAND_ADDRESS, POST_INC unimpl.");
		  fprintf (file, "(%s+%s)+",
			   reg_names [REGNO(reg)], reg_names [REGNO(offset)]);
		}
		else if (GET_CODE(inc) == PRE_DEC) {
		  if (!USE_PRE_DECREMENT)
	            abort_with_insn (addr,
				     "PRINT_OPERAND_ADDRESS, PRE_DEC unimpl.");
		  fprintf (file, "(%s+%s)^-",
			   reg_names [REGNO(reg)], reg_names [REGNO(offset)]);
		}
		else if (GET_CODE(inc) == POST_DEC) {
		  if (!USE_POST_DECREMENT)
		    abort_with_insn (addr,
				     "PRINT_OPERAND_ADDRESS, POST_DEC unimpl.");
		  fprintf (file, "(%s+%s)-",
			   reg_names [REGNO(reg)], reg_names [REGNO(offset)]);
		}
		else
		  abort_with_insn (addr, "INC_DEC target not allowed.");
	      }
	      else if (GET_CODE (offset) == REG)
		abort_with_insn (addr, "PRINT_OPERAND_ADDRESS, 2 regs");
	      else if (!CONSTANT_P (offset))
	    	abort_with_insn(addr, "PRINT_OPERAND_ADDRESS, illegal insn");
	      else if (REGNO (reg) == ARG_POINTER_REGNUM)
	        abort_with_insn (addr, "Arg pointer not eliminated.");
...
int
ss_output_external (file, decl, name)
     FILE *file;
     tree decl;
     char *name;
{
...

/* Emit a new filename to a stream.  If this is SS ECOFF, watch out
   for .file's that start within a function.  If we are smuggling stabs, try to
   put out a SS ECOFF file and a stab.  */

void
ss_output_filename (stream, name)
     FILE *stream;
     char *name;
{
  ...
  if (first_time)
    {
      ...
      fprintf (stream, "\n");
      /* This tells ss-tfile that stabs will follow.  */
	  ...
	  if (!file_in_function_warning)
	    {
	      ...
	      warning ("SS ECOFF format does not allow changing filenames within functions with #line");
	    }
...
/* Emit a linenumber.  For encapsulated stabs, we need to put out a stab
   as well as a .loc, since it is possible that SS ECOFF might not be
   able to represent the location for inlines that come from a different
   file.  */

void
ss_output_lineno (stream, line)
     FILE *stream;
     int line;
{
  ...
  if (dslots_number_nops > 0)
    {
      ...
      if (length == 0
	  || (ss_load_reg  != (rtx)0 && reg_mentioned_p (ss_load_reg,  pattern))
	  || (ss_load_reg2 != (rtx)0 && reg_mentioned_p (ss_load_reg2, pattern))
	  || (ss_load_reg3 != (rtx)0 && reg_mentioned_p (ss_load_reg3, pattern))
	  || (ss_load_reg4 != (rtx)0 && reg_mentioned_p (ss_load_reg4, pattern)))
	/* tma, nops not needed */
...
/* Output at beginning of assembler file.
   ...
   This is needed because the SS assembler is evidently one pass,
   and if it hasn't seen the relevant .comm/.lcomm/.extern/.sdata
   declaration when the code is processed, it generates a two
   instruction sequence.  */

void
ss_asm_file_start (stream)
     FILE *stream;
{
  ASM_OUTPUT_SOURCE_FILENAME (stream, main_input_filename);

  /* Versions of the SS assembler before 2.20 generate errors
  ...
  if (TARGET_SS_AS && optimize && flag_delayed_branch)
...
/* Emit either a label, .comm, or .lcomm directive, and mark
   ...
   for it in ss_asm_file_end.  */
extern int flag_kp_alloc;
extern FILE *xo_dump_file;

void
ss_declare_object (stream, name, init_string, final_string, size, align)
     FILE *stream;
     char *name;
     char *init_string;
     char *final_string;
     ...
     int align;
{
  ...
  /* ":\n", ",%u\n", ",%u\n" */
  fprintf (stream, final_string,
	   size, (int)floor_log2(align/BITS_PER_UNIT));

  if (TARGET_GP_OPT && ss_section_threshold != 0)
    {
     ...
     rtx insn;
{
  ...
  switch (code)
	...
	return 1;
      ...
      if (regno != SS_TEMP1_REGNUM && regno != SS_TEMP2_REGNUM)
	return 0;
     ...
     int size;			/* # of var. bytes allocated */
{
  ...
  mask		 = 0;
  fmask		 = 0;
  extra_size	 = SS_STACK_ALIGN (((TARGET_ABICALLS) ? UNITS_PER_WORD : 0));
  var_size	 = SS_STACK_ALIGN (size);
  args_size	 = SS_STACK_ALIGN (current_function_outgoing_args_size);

  /* The SS 3.0 linker does not like functions that dynamically
  ...
  gp_reg_rounded = SS_STACK_ALIGN (gp_reg_size);
  total_size += gp_reg_rounded + fp_reg_size;

  /* align total size again... */
  if (TARGET_STR_STACK)
    total_size = (total_size + (ss_stack_alignment/BITS_PER_UNIT-1))
      & ~(ss_stack_alignment/BITS_PER_UNIT-1);
...
/* Common code to emit the insns (or to write the instructions to a file)
   ...
   Other parts of the code assume that SS_TEMP1_REGNUM (aka large_reg)
...
ss_expand_prologue ()
{
  ...
  if (tsize > 0)
      ...
      if (!TARGET_ABICALLS)
	{
	  if (tsize > 32767)
	    {
	      tmp_rtx = gen_rtx (REG, SImode, SS_TEMP1_REGNUM);
     ...
     int size;
{
  long tsize;
  char *sp_str = reg_names[STACK_POINTER_REGNUM];
  char *t1_str = reg_names[SS_TEMP1_REGNUM];
  rtx epilogue_delay = current_function_epilogue_delay_list;
  int noreorder = !TARGET_GAS || (epilogue_delay != 0);
  ...
  /* tma, better not try to fill any delay slots */
  assert(epilogue_delay == 0);
  ...
  if (!noepilogue)
      ...
      if (tsize > 32767)
	{
	  ...
	  tmp_rtx = gen_rtx (REG, Pmode, SS_TEMP1_REGNUM);
	}
      ...
      if (noreorder)
	{
	  ...
	  /* tma, should never get here, as there is no re-ordering */
	  abort();
...
/* Define the number of delay slots needed for the function epilogue.
   ...
   On the ss, we need a slot if either no stack has been allocated,
   or the only register saved is the return register.  */

/* tma, no delay slots on the SimpleScalar */
#if 0
int
ss_epilogue_delay_slots ()
{
  if (!current_frame_info.initialized)
...
ss_init(void)
{
  if (main_input_filename) {
    char *p;
    main_label = (char *)xmalloc (strlen(main_input_filename)+1);
    strcpy(main_label, main_input_filename);
    strip_off_ending (main_label, strlen(main_label));
    /* turn the filename into a legal identifier */
    for (p=main_label; *p; p++)
      if (*p == '-' || *p == '.' || *p == '$' || *p == '/')
        *p = '_';
  }
  else
     ...
     rtx x;
{
  ...
  if (GET_MODE_SIZE (mode) <= ss_section_threshold
      && ss_section_threshold > 0)
    sdata_section ();
  else
    rdata_section ();
     ...
     int reloc;
{
  ...
  if (size > 0 && size <= ss_section_threshold)
    sdata_section ();
...
/* maximum type alignment in bytes */
int ss_max_alignment = 32*8;	/* 32 bytes is the default */
char *ss_max_align_string = NULL;
int ss_stack_alignment = 64*8;	/* 64 bytes is the default */
char *ss_stack_align_string = NULL;

/* pre- and post- inc/dec options */
char *ss_inc_dec_opt_string = NULL;

int ss_inc_dec_opt = 0;		/* default - non enabled */

/* A C expression to compute the alignment given to a
   constant that is being placed in memory.  CONSTANT is the constant
   and ALIGN is the alignment that the object would ordinarily have.
   The value of this macro is used instead of that alignment to align
   the object. */
int
ss_constant_alignment(tree expr, int align, int normalAlign)
{
  return ss_data_alignment(TREE_TYPE(expr), align, normalAlign);
}

/* A C expression to compute the alignment for a static
   variable.  TYPE is the data type, and ALIGN is the alignment that
   the object would ordinarily have.  The value of this macro is used
   instead of that alignment to align the object. */
int
ss_data_alignment(tree type, int align, int normalAlign)
{
  int eltAlign = 0;

  if (TREE_CODE(type) == ARRAY_TYPE || TREE_CODE(type) == POINTER_TYPE)
    eltAlign = int_size_in_bytes(TREE_TYPE(type));

  return MIN(ss_max_alignment,
	     MAX((1 << ceil_log2(eltAlign)) * BITS_PER_UNIT, normalAlign));
}

/* return the absolute value of the address */
int
folded_offset(rtx offset)
{
 restart:
  ...
  case LABEL_REF:
  case CODE_LABEL:
    /* anything (much) larger than 255 is fine */
    return 1024*1024;
  ...
  case CONST:
    return folded_offset(XEXP(offset, 0));

  case CONST_DOUBLE:
    abort();

  case PLUS:
    return folded_offset(XEXP(offset, 0)) + folded_offset(XEXP(offset, 1));

  case MINUS:
    offset = simplify_subtraction(offset);
    if (GET_CODE(offset) != MINUS)
      goto restart;
    return folded_offset(XEXP(offset, 0)) - folded_offset(XEXP(offset, 1));
